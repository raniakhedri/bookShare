...
class StoreReviewRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'review_text' => 'required|string|min:1|max:5000',
            'reading_context' => 'nullable|string|max:500',
            'is_spoiler' => 'boolean',
            'content_warnings' => 'nullable|string|max:200',
            'photos' => 'nullable|array|max:5',
            'photos.*' => 'image|mimes:jpeg,png,jpg,gif|max:2048' // 2MB max per image
        ];
    }
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            // Custom validation: If condition_rating is provided, it should be reasonable
            if ($this->condition_rating && $this->overall_rating) {
                $diff = abs($this->condition_rating - $this->overall_rating);
                if ($diff > 3) {
                    $validator->errors()->add('condition_rating', 
                        'Condition rating seems inconsistent with overall rating.');
                }
            }

            // Custom validation: Ensure spoiler reviews have appropriate warnings
            if ($this->is_spoiler && empty($this->content_warnings)) {
                $validator->errors()->add('content_warnings', 
                    'Please provide content warnings for spoiler reviews.');
            }
        });
    }
}