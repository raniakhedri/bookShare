...
@section('title', 'Edit Review: ' . $review->book->title)
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-8">
            ...
            <div>
                ...
                <h2 class="text-xl text-gray-700 font-medium">{{ $review->book->title }}</h2>
        ...
        <div class="bg-white rounded-lg shadow-sm border p-8">
            <form action="{{ route('reviews.update', $review->review_id) }}" method="POST" enctype="multipart/form-data" id="reviewForm">
                @csrf
                @method('PUT')
...
<div class="mb-6">
    ...
    @enderror
                        ...
                        <div class="flex items-center space-x-1">
                            @for($i = 1; $i <= 5; $i++)
                                <button type="button" 
                                        ...
                                        class="star-button {{ $i <= old('content_rating', $review->content_rating) ? 'text-yellow-400' : 'text-gray-300' }} hover:text-yellow-400 transition duration-200"
                                        data-rating="{{ $i }}"
                        ...
                        <div class="flex items-center space-x-1">
                            @for($i = 1; $i <= 5; $i++)
                                <button type="button" 
                                        ...
                                        class="star-button {{ $i <= old('condition_rating', $review->condition_rating) ? 'text-yellow-400' : 'text-gray-300' }} hover:text-yellow-400 transition duration-200"
                                        data-rating="{{ $i }}"
                ...
                <div class="mb-6">
                    <div class="flex items-center">
                        <input type="checkbox" 
                               ...
                               id="is_spoiler"
                               value="1"
                               {{ old('is_spoiler', $review->is_spoiler) ? 'checked' : '' }}
                ...
                <!-- Content Warnings -->
                <div class="mb-6" id="contentWarningsDiv" style="display: {{ old('is_spoiler', $review->is_spoiler) ? 'block' : 'none' }};">
                ...
                @if($review->photo_urls && count($review->photo_urls) > 0)
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-900 mb-2">Current Photos</label>
                    ...
                    <div class="flex space-x-3">
                        ...
                        <form action="{{ route('reviews.destroy', $review->review_id) }}" method="POST" class="inline">
                            ...
                            <button type="submit" 
                                    ...
                                    class="px-4 py-2 border border-red-300 rounded-md shadow-sm text-sm font-medium text-red-700 bg-red-50 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition duration-200">
                                Delete Review
                            </button>
                        </form>
                    </div>
...
function initializeStarRatings() {
    ['overall_rating', 'content_rating', 'condition_rating'].forEach(field => {
        const value = document.getElementById(field).value;
        if (value) {
            setRatingDisplay(field, parseInt(value));
        }
    });
}
...
function setRatingDisplay(field, rating) {
...
function removeCurrentPhoto(index) {
    ...
    document.getElementById('remove_photos').value = JSON.stringify(removedPhotos);
    
    // Hide the photo visually
